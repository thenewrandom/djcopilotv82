        ...
        fetch('/api/voice-tts', {
          ...
          .then(blob => {
            ...
            audio.onerror = () => {
              ...
              setNowPlayingStr(`DJ ${persona.name}: realistic voice playback failed`);
              finish();
            };
            audio.play().catch(() => {
              URL.revokeObjectURL(url);
              setNowPlayingStr(`DJ ${persona.name}: enable audio playback and retry`);
              finish();
            });
          })
          .catch(error => {
            console.error('Realistic DJ voice generation failed:', error);
            setNowPlayingStr(`DJ ${persona.name}: realistic voice unavailable — check ElevenLabs configuration`);
            finish();
          });
        return;
      }

      // Built-in personas are expected to have a real provider voice ID.
      // Browser speech is intentionally reserved for personas without a configured provider voice.
      speakBrowser();
