    ...
    if (p.voiceId) {
      fetch('/api/voice-tts', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ voiceId: p.voiceId, text: sample, speed: p.rate }) })
        ...
        .then(blob => {
          ...
          audio.onerror = () => { URL.revokeObjectURL(url); setAuthNotification(`Realistic voice preview failed for ${p.name}. Check your ElevenLabs configuration.`); };
          audio.play().catch(() => setAuthNotification(`Click the browser audio permission/playback prompt, then preview ${p.name} again.`));
        })
        .catch((error) => {
          console.error(error);
          setAuthNotification(`Realistic voice preview unavailable for ${p.name}. Check ELEVENLABS_API_KEY in Vercel.`);
