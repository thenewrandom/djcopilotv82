export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const response = await fetch(`https://api.elevenlabs.io/v1/text-to-speech/${encodeURIComponent(voiceId)}`, {
      ...
      body: JSON.stringify({ text: String(text).slice(0, 5000), model_id: process.env.ELEVENLABS_TTS_MODEL || 'eleven_multilingual_v2', voice_settings: { stability: Math.max(0, Math.min(1, Number(stability))), similarity_boost: Math.max(0, Math.min(1, Number(similarityBoost))), style: Math.max(0, Math.min(1, Number(style))), speed: Math.max(0.7, Math.min(1.2, Number(speed))) } })
    });
    if (!response.ok) {
